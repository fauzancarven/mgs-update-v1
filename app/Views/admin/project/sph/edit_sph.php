...
<div class="modal fade" id="modal-edit-penawaran" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="1"  aria-labelledby="modal-edit-penawaran-label" style="overflow-y:auto;" data-menu="project">
    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="modal-title fs-5 fw-bold" id="modal-edit-penawaran-label">Ubah Penawaran</h2>
...
$(document).ready(function() {
    ...
    $('#SphDate').daterangepicker({
        ...
        "startDate": moment('<?=$project->SphDate ?>'),
        "endDate":  moment('<?=$project->SphDate ?>'),
        locale: {
            format: 'DD MMMM YYYY'
        }
    });
    ...
    /* EDIT MODE
    */ 
    $("#SphCode").val('<?= $project->SphCode ?>');

    // CUSTOMER
    $("#SphCustName").val('<?= $project->SphCustName ?>');
    $("#SphCustTelp").val('<?= $project->SphCustTelp ?>');
    $("#SphAddress").val('<?= $project->SphAddress ?>');
    ...
    <?php
        if($project->SphRef == 0){  
            ...
            echo "$('#SphRef').append(optionref).trigger('change');"; 
        }else{
            echo "var optionref = new Option('".$project->SphRefCode."' , ".$project->SphRef.", true, true);";
            echo '$(optionref).data("type","'.$project->SphRefType.'");'; 
            echo "$('#SphRef').append(optionref).trigger('change');"; 
    ...
    // Delivery  
    $("input[name='SphDelivery'][value='<?= $project->SphDelivery ?>'").prop("checked", true); 
    ...
    $(".template-footer").each(function(index, el){
        ...
        $(btnsaveas).click(function(){
            $("#modal-edit-penawaran").modal("hide"); 
            Swal.fire({
    ...
    $("#btn-add-penawaran").click(function(){
        ...
        $.ajax({ 
            dataType: "json",
            method: "POST",
            url: "<?= base_url() ?>action/edit-data-penawaran/<?= $project->SphId ?>", 
            data:{
                "header":header,
                "detail":detail, 
            },
            success: function(data) {    
                ...
                if(data["status"]===true){
                    Swal.fire({
                        ...
                        $("#modal-edit-penawaran").modal("hide");      
                        if($("#modal-edit-penawaran").data("menu") =="Penawaran"){
