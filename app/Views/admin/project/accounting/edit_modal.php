<div class="modal fade" id="modal-edit-accounting" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"  aria-labelledby="modal-add-accounting-label" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="modal-title fs-5 fw-bold" id="modal-add-accounting-label">Edit Dana Modal</h2>
                ...
                <div class="mb-1"> 
                    <div class="row">
                        <div class="col-6">
                            ...
                            <select class="form-select form-select-sm" style="width:100%" id="type-accounting">
...
<script>
    $('#date-accounting').daterangepicker({
        ...
        "startDate": moment('<?= $project->AccDate?>'),
        "endDate":  moment('<?= $project->AccDate?>'),
        locale: {
            format: 'DD MMMM YYYY'
        }
    ...
    $("#total-accounting").val('<?= $project->AccTotal?>')
    $("#comment-accounting").val('<?= $project->AccComment?>')
    ...
    $("#desc-accounting").val('<?= $project->AccName?>')
    $('#type-accounting option[value="<?= $project->AccType?>"]').attr("selected", "selected"); 
    ...
    $("#btn-add-accounting").click(function(){
        ...
        $.ajax({ 
            dataType: "json",
            method: "POST",
            url: "<?= base_url() ?>action/edit-data-project-accounting/<?= $project->AccId?>", 
            data:{
            ...
            success: function(data) {    
                ...
                if(data["status"]===true){
                    Swal.fire({
                        ...
                        $("#modal-edit-accounting").modal("hide");
